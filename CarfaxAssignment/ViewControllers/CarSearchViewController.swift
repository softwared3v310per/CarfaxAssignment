...
class CarSearchViewController: UIViewController {
    ...
    func getCarImage(imageURL: String?, cell: CarSearchTableViewCell) {
        if let imageURL = imageURL, let url = URL(string: imageURL) {
            self.viewModel.getPhotoData(url: url) { photoData in
                DispatchQueue.main.async {
                    if let photoData = photoData {
                        cell.carImageView.image =  UIImage(data: photoData)
                    } else {
...
extension CarSearchViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        self.getCarImage(imageURL: carListings[indexPath.row].images?.firstPhoto.medium, cell: cell)
